...
def readtoscipp(filename):
    ...
    pos = (ampl_a + ampl_b - (ampl_c + ampl_d) ) / (ampl_a + ampl_b + ampl_c + ampl_d)
    straw = (ampl_b + ampl_d - (ampl_a + ampl_c)) / (ampl_a + ampl_b + ampl_c + ampl_d)
...
def load_and_save(args):
    ...
    for i in range(8):
        ...
        xi = i % 2
        cax = ax[yi, xi]
        grp.hist(amplitude_a=400, amplitude_b=400).plot(aspect=1.,norm='log', ax=cax)
        cax.title.set_text(f'Tube {i}')
        cax.set_xlim(-1, args.zoom)
        cax.set_ylim(-1, args.zoom)
        cax.yaxis.tick_left()
        cax.yaxis.set_label_position('left')
        if i <= 5:
            cax.set(xlabel='', ylabel='ampl b')
        else:
            cax.set(xlabel='ampl a', ylabel='ampl b')

    plt.suptitle(f'Amplitude A vs Amplitude B\nRing: {id2chr(args.ring)}, FEN: {id2chr(args.fen)}, Tubes 0 - 8', size='18')
    plt.savefig(os.path.join(args.outdir, f'ampavsampb_{id2chr(args.ring)}_{id2chr(args.fen)}.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-z','--zoom', type=int, default=32000, help='Max amplitude')
